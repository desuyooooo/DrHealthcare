...
while ($tag = mysqli_fetch_array($tags)){
  echo '<span style="font-size:'.$t.'px;"><li><a href="'.$_SESSION['from'].'?tag='.$tag['tag'].'" title="'.$tag['tag'].'">#'.$tag['tag'].'</a></li></span>';
  $t-=3;
}
?>
               </ul>
...
while ($row = mysqli_fetch_array($cat)){
  echo '<li><a href="'.$_SESSION['from'].'?id='.$row['catid'].'" title="'.$row['catdesc'].'">'.$row['catname'].'</a></li>';
}
?>
               </ul>
            </div>
